...
def load_middlebury_data(datadir):
    ...
    BBox = {
        "templeRing": np.array(
            [[-0.023121, -0.038009, -0.091940], [0.078626, 0.121636, -0.017395]]
        )
    }

    print(f"Loading {datadir}")
    camera_fn = [
        osp.join(datadir, fn) for fn in os.listdir(datadir) if fn.endswith("_par.txt")
    ]
    assert len(camera_fn) == 1, "camera not found or duplicated"
    viz_fn = [
        osp.join(datadir, fn) for fn in os.listdir(datadir) if fn.endswith("_ang.txt")
    ]
