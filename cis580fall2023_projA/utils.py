...
def add_coordinate(scene, R, T, axis_len=0.05, sections=6, ratio=20):
    ...
    mx.visual.vertex_colors = np.ones(
        mx.vertices.shape) * np.array([[1.0, 0.0, 0.0]])
    ...
    my.visual.vertex_colors = np.ones(
        my.vertices.shape) * np.array([[0.0, 1.0, 0.0]])
    ...
    mz.visual.vertex_colors = np.ones(
        mz.vertices.shape) * np.array([[0.0, 0.0, 1.0]])
...
def viz_camera_poses(DATA):
    ...
    for data in DATA:
        scene = add_coordinate(
            scene, data["R"].T, -(data["R"].T @ data["T"][:, None])[:, 0])
